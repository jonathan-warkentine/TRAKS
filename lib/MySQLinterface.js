...
module.exports = class MySQLinterface{
    ...
    async chooseDatabase(){
        ...
        await inquirer.prompt({
            ...
            type: 'list',
            choices: [...databases.map(database => this.tools.utils.objValues(database)).flat(), '[ + NEW DATABASE ]'],
        })
        .then(async answer => {
    ...
    modes = {
        ...
        async ADD(parent){
            await inquirer.prompt({
                ...
                validate: answer => /^[0-9a-zA-Z]+$/.exec(answer)? true: 'Database Name Must be Alphanumeric'
            })
            .then(async answer => {
