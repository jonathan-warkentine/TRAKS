        ...
        return await inquirer.prompt(
            {
                ...
                validate: answer => /^[a-z0-9]+$/.exec(answer)? true: 'Table Names Must be Alphanumeric'
            }
        )
        .then(async answer => {
            ...
            await inquirer.prompt(
                {
                    ...
                    validate: answer => typeof answer == 'string' && /^[a-z0-9]+$/.exec(answer)? true: 'Name Must be Alphanumeric'
                }
            )
            .then(async answer => {
                newTableName = answer.rename;
                await parent.connection.promise().query(`ALTER TABLE ${table} RENAME TO ${newTableName}`);
                console.log(chalk.green('\n Rename Successful! \n'));
            });

            await parent.modeSelect(newTableName);
        },
