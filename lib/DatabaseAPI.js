...
class DatabaseAPI {
    ...
    async modeSelect(table){
        await inquirer.prompt({
            ...
            choices: [`OPEN TABLE ${table}`,`PRINT TABLE ${table}`, `RENAME TABLE ${table}`, `ADD_FIELD to ${table}`, `REMOVE_FIELD from ${table}`, `DELETE TABLE ${table}`, `[ back ]`],
    ...
    dbAPI = {
        ...
        async AGAIN(parent){
            ...
            // await parent.parent.modeSelect(this.database);
        }
    }
}
