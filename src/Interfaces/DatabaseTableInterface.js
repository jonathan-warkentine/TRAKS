...
const DatabaseTableTools = require('../Tools/DatabaseTableTools');
...
module.exports = class DatabaseTableInterface extends DatabaseInterface {
        ...
        await inquirer.prompt(
            {
                ...
                choices: async () => {
                    ...
                    return [...rows? rows.map(row => JSON.stringify(row).replace(/\"/g, " ")): [],'[ + ] NEW ROW', '[ <- ] back'];
                }
            }
        )
        .then(async answer => {
        ...
        if (colInfo.length){ //if there's no non-pk fields, returns null
            ...
            for await (let field of this.tools.utils.objValues(fieldSelect)){
                await inquirer.prompt([
                {
                    ...
                    validate: answer => this.tools.utils.validateType(answer, field.Type) && /^[0-9a-zA-Z _]+$/.exec(answer)? true: `This field only accepts input of type ${field.Type}!`
                },
                {
                    when: () => fkEval.foreign_key == field.Field,
                    name: field.Field,
                    ...
                    choices: async () => {
                        ...
                        return options.map(option => JSON.stringify(option).replace(/\"/g, " "));
                    },
                    message: () => `Update Value for Field "${field.Field.toUpperCase()}"`,
                    filter: (answer) => {
        ...
        for await (let field of this.tools.utils.objValues(colInfo)){
            await inquirer.prompt(
                [{
                    ...
                    validate: answer => this.tools.utils.validateType(answer, field.Type)? true: `This field only accepts input of type ${field.Type}!`
                },
                {
                    when: () => fkEval.foreign_key == field.Field,
                    name: field.Field,
    ...
    mode = { //stored in an object for reference purposes
        ...
        async DELETE(parent, row) {
            await parent.tools.deleteRow(row[parent.primaryKey], parent.primaryKey);
            await parent.chooseRow();
        },
