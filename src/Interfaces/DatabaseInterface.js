...
module.exports = class DatabaseInterface extends MySQLinterface{
            ...
            more = await inquirer.prompt([
                {
                    ...
                    validate: answer => /^[0-9a-zA-Z_]+$/.exec(answer)? true: "Column Names Must be Alphanumeric" //include special character restrictions
                },
                {
                    name: 'newColType',
                    type: 'list',
        ...
        return await inquirer.prompt(
            {
                ...
                validate: answer => /^[0-9a-zA-Z_]+$/.exec(answer)? true: 'Table Names Must be Alphanumeric'
            }
        )
    ...
    mode = {
            ...
            await inquirer.prompt(
                {
                    ...
                    validate: answer => typeof answer == 'string' && /^[0-9a-zA-Z_]+$/.exec(answer)? true: 'Name Must be Alphanumeric'
                }
            )
            .then(async answer => {
        ...
        async DELETE(parent, table){
            ...
            .then(async answer => answer.sure == table.toLowerCase()? await parent.tools.deleteTable(table): parent.modeSelect(table));
        },
